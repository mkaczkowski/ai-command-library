...
const COMMAND_SOURCE_ROOT = path.join(PACKAGE_ROOT, 'library/commands');
...
async function buildDefaultMappings(sourceRoot, providerConfig) {
...
function replaceMarkdownPlaceholders(content, {providerConfig, destinationDisplay, commandsRootDisplay}) {
    ...
    replaced = replaced.replace(PATH_PLACEHOLDER_PATTERN, (_, rawKey) => {
        ...
        if (key !== 'commandsRoot') {
            throw new Error(`Unknown path placeholder '${key}' for provider '${providerConfig.id ?? 'unknown'}'.`);
        }
        if (!commandsRootDisplay) {
            throw new Error(`Provider '${providerConfig.id ?? 'unknown'}' does not specify a commandsRoot display path.`);
        }
...
export async function main(argv = process.argv.slice(2)) {
    ...
    const commandsRootDisplay = args.destination ?? providerConfig.defaultTargetDir;
