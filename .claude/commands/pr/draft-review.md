...
```bash
node .claude/commands/pr/scripts/create-pr-review.js --input=tmp/pr-[PR_NUMBER]-review-comments.json --pr=[PR_NUMBER]
```

**Note:** If it fails because of existing pending reviews, ask the user to add `--discard-existing` flag. If the user agrees:

```bash
node .claude/commands/pr/scripts/create-pr-review.js --input=tmp/pr-[PR_NUMBER]-review-comments.json --pr=[PR_NUMBER] --discard-existing
```
