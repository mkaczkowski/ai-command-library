...
node "$REPO_ROOT/.claude/commands/pr/scripts/fetch-pr-context.js" --pr=[PR_NUMBER] --output="$REPO_ROOT/tmp/pr-[PR_NUMBER]-context.json"
```
...
```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
node "$REPO_ROOT/.claude/commands/pr/scripts/create-pr-review.js" --input="$REPO_ROOT/tmp/pr-[PR_NUMBER]-review-comments.json" --pr=[PR_NUMBER]
```
...
```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
node "$REPO_ROOT/.claude/commands/pr/scripts/create-pr-review.js" --input="$REPO_ROOT/tmp/pr-[PR_NUMBER]-review-comments.json" --pr=[PR_NUMBER] --discard-existing
```
